...
public class CreateFlux {
    ...
    public static void main(String[] args) {
        ...
        /** 通过generate创建 **/
        Flux.generate(sink -> {sink.next(1);sink.complete();});
        ...
        Flux.generate(AtomicInteger::new, (state, sink) -> {long i = state.getAndIncrement();sink.next(i);if (i == 10) {sink.complete();} return state;}, state -> state.set(0) // 最终清理
